...
function initAllCarousels() {
    const carousels = document.querySelectorAll('.pc-carousel-wrapper');
    carousels.forEach(carousel => {
        // Skip if already initialized (prevents duplicate instances)
...
class ProductCarousel {
    ...
    handleTouchMove(e) {
        ...
        if (!this.isDragging && !this.isScrolling) {
            if (Math.abs(deltaX) > this.settings.scrollLockThreshold && Math.abs(deltaX) > Math.abs(deltaY)) {
                this.isDragging = true;
                this.isScrolling = false;
                e.preventDefault();
            } else if (Math.abs(deltaY) > this.settings.scrollLockThreshold) {
                this.isDragging = false;
